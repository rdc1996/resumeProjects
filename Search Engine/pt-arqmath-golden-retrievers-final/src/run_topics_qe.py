...
def process_args():
    ...
    parser.add_argument('-b', '--base', help="run only baseline model", action="store_true" )
...
def main():
    ...
    qe_pipeline = pt.rewrite.SDM() >> bm25_engine >> prime_transformer

    systems = [ bm25_pipeline ]
    sys_names = [ weight_model ]
    if not args.base:
        systems.append(qe_pipeline)
        sys_names.append("SDM-BM25")
