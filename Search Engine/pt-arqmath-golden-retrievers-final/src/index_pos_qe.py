...
def create_XML_index( file_list, indexName, token_pipeline="Stopwords,PorterStemmer", formulas=False, debug=False, pos_qe=False):
...
def process_args():
    ...
    parser.add_argument("-p", "--pos", help="preprocess with position and query expansion", action="store_true" )
...
def main():
    ...
    if not args.math or args.mathpost:
        post_index = create_XML_index(
            ...
            token_pipeline=args.tokens, debug=args.debug, pos_qe=args.pos )
